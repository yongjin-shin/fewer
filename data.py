...
class Preprocessor:
    ...
    def distribute_data(self, server):
        ...
        dataset_locals, locals_lb = self.make_data_for_local(dataset_train)
        dataset_server, dataset_test, valid_lb = self.make_data_for_server(dataset_test)
        server.get_data(dataset_server, dataset_locals, dataset_test, locals_lb, valid_lb)

    def make_data_for_server(self, dataset_test):
        labels = dataset_test.targets

        val_idx = []

        for ul in np.unique(labels):
            idx_by_label = np.where(labels == ul)[0]
            _idx = np.random.choice(idx_by_label,
                                    size=int(len(idx_by_label) * self.args.nb_server_data),
                                    replace=False)
            val_idx += _idx.tolist()

        dataset_server = Subset(dataset_test, val_idx)

        server_labels = []
        for i in range(dataset_server.__len__()):
            server_labels.append(dataset_server.__getitem__(i)[1])

        valid_lb = {}
        for ul in np.unique(server_labels):
            idx_by_label = np.where(server_labels == ul)[0]
            valid_lb[ul] = idx_by_label.tolist()

        test_idx = list(set(np.arange(len(labels)).tolist()) - set(val_idx))
        dataset_test = Subset(dataset_test, test_idx)

        print(f" Center length: {dataset_server.__len__()}\n"
              f" Test length: {dataset_test.__len__()}\n")
        return dataset_server, dataset_test, valid_lb

    def make_data_for_local(self, dataset_train):
        """클라이언트에 얼마나 데이터 분배할지 결정. Train 데이터만 사용한다"""

        # non iid 데이터를 만들어준다
        locals_idx, locals_lb = self.make_non_iid(dataset_train.targets)
        ...
        for i in range(self.args.nb_devices):
            ...
            validsets_label.append(locals_lb.pop())

        print(f"\nDataset Length\n"
              f" Local length: {len(datasets_local)} x {datasets_local[0].__len__()}")
        
        return datasets_local, validsets_label
    ...
    def make_non_iid(self, labels):
        ...
        if self.args.iid or self.args.nb_devices == 1:
            ...
            lb.append(np.unique(labels).sort())
        ...
        print(f'Remaining # of Training Data samples: {len(remained_idx)}')

        return deque(idx), deque(lb)
